...
class Date extends AbstractField {
	...
	public function script() {
		?>
		<script src="fields/Date/js/date.js"></script>
		<?php
	}
	...
	public function render($data) {
		...
		<p class="extra-field">
			...
			<input
				...
				data-date-format="<?php echo $this->format[2]; ?>"
				<?php echo ($this->min != null) ? ' data-date-min="'.$this->min[0].'"' : ''; ?>
				<?php echo ($this->max != null) ? ' data-date-max="'.$this->max[0].'"' : ''; ?>
				type="text"
				name="<?php echo $this->name; ?>"
				id="<?php echo $this->name; ?>"
				value="<?php if($value !== null) echo $value; ?>" />
	...
	public function get_value_for_database($data) {
		...
		if ($value != null) {
			$format = $this->format[0];
			$date = \DateTime::createFromFormat($format, $value);
			$value = date('Y-m-d', $date->getTimestamp());
		}
